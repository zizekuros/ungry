/*
  # Create Subscription Limit Checker Function

  1. Subscription Plans
    - free: 15 items per week (Monday-Sunday), 1 active shopping list
    - standard: 50 items per week (Monday-Sunday), 3 active shopping lists
    - unlimited: unlimited items and shopping lists
    - admin: unlimited items and shopping lists (for administrators)

  2. Subscription Periods
    - monthly: subscription renews every month, user gets full day on renewal date
    - annual: subscription renews every year, user gets full day on renewal date
    
  3. Period Calculation Examples
    - Monthly: renewed on Sep 5 2025 → valid until Oct 5 2025 23:59:59
    - Monthly: renewed on Jan 31 2025 → valid until Feb 28 2025 23:59:59 (handles month lengths)
    - Annual: renewed on Sep 5 2025 → valid until Sep 5 2026 23:59:59

  4. Function Behavior
    - Checks if user can add items or create lists based on subscription
    - Returns subscription status (active/expired) but does NOT auto-downgrade
    - Only limits: adding new items and creating new lists
    - Does NOT limit: item removal, bought status changes, or list deletion
    - Item limit uses current week (Monday 00:00:00 to Sunday 23:59:59) from user_item_analytics
    - List limit counts active lists (deleted_at IS NULL)
    - Week resets every Monday at midnight
*/

-- Function to check if user can perform an action based on subscription limits
...
BEGIN
  ...
  IF action_type = 'create_list' THEN
    ...
    -- Count current active shopping lists
    SELECT COUNT(*) INTO current_active_lists
    FROM shopping_lists
    WHERE (owner_id = user_id_param OR id IN (
      SELECT list_id FROM list_members WHERE user_id = user_id_param
    ))
...
GRANT EXECUTE ON FUNCTION public.check_user_subscription_limit(uuid, text) TO authenticated;
