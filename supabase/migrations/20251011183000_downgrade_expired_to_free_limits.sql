/*
  # Downgrade expired subscriptions to free tier limits

  When a subscription expires, instead of blocking all actions:
  - Apply free tier limits (15 items/week, 1 list)
  - Keep original subscription metadata unchanged (for billing/renewal)
  - User can still use the app with free tier restrictions
  
  Subscription periods:
  - monthly: expires after 1 month
  - annual: expires after 1 year
  - forever: never expires (for admin/special cases)
*/
...
DECLARE
  ...
  effective_plan text; -- The plan to actually apply (may differ from subscription_plan if expired)
BEGIN
  ...
  ELSE
    ...
    IF subscription_plan != 'free' AND subscription_renewed IS NOT NULL AND subscription_period IS NOT NULL THEN
      -- Check if subscription has forever period (never expires)
      IF subscription_period = 'forever' THEN
        -- Forever subscription never expires
      ...
      ELSE
        ...
        -- If expired, apply free tier limits but keep original plan in metadata
  ...
  -- Set limits based on EFFECTIVE plan (not subscription plan)
  CASE effective_plan
    WHEN 'free' THEN
  ...
  IF action_type = 'create_list' THEN
    ...
    -- Count current shopping lists (no deleted_at check - tables use hard deletion)
    ...
    IF current_active_lists >= max_active_lists THEN
      RETURN jsonb_build_object(
        ...
        'message', format('You have reached the maximum of %s active list(s) for your %s plan', max_active_lists, effective_plan)
      );
    ELSE
      RETURN jsonb_build_object(
        'allowed', true,
  ...
  ELSIF action_type = 'add_item' THEN
    ...
    IF items_added_this_period >= max_items_per_week THEN
      RETURN jsonb_build_object(
        ...
        'message', format('You have reached the maximum of %s items per week for your %s plan', max_items_per_week, effective_plan)
      );
    ELSE
      RETURN jsonb_build_object(
        'allowed', true,
